...
const App = () => {
  // We're going to watch my favourite movie,
  // in our BOYFRIEND's favourite genre.
  // (Terror at Jarry Park)
  const movie = useSelector((state) => {
    const boyfriendFavoriteGenre = state.boyfriendFavoriteGenre;
    return state.myFavoriteMovies[boyfriendFavoriteGenre];
  });
...
const UserProfile = () => {
  ...
  const streetAddress = useSelector((state) => {
    const { line1, line2 } = street.address;
    if (line2) {
      return `${line1} ${line2}`;
    }
    return line1;
...
const OnlineUsers = () => {
  ...
  const onlineUsers = useSelelctor((state) => {
